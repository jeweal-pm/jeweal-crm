    ...
    <section class="crm-page">
        <div class="container-fluid">
            ...
            @forelse($sequence->steps as $step)
                <section class="crm-panel mb-3">
                    <div class="email-panel-head"><div><h3 class="crm-panel-title">Step {{ $step->step_number }}</h3><div class="email-panel-copy">Sent {{ $step->delay_value }} {{ $step->delay_unit }} after the previous step.</div></div><form method="post" action="{{ route('email.sequences.steps.destroy', [$sequence->id, $step->id]) }}" onsubmit="return confirm('Remove Step {{ $step->step_number }}?');">@csrf @method('delete')<button class="btn btn-sm btn-outline-danger" type="submit" title="Remove step"><i class="fas fa-trash"></i></button></form></div>
