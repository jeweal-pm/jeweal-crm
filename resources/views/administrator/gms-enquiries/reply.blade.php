...
@section('head')
@section('title', 'Reply GMS Enquiry')
...
<section class="content crm-page">
    <div class="container-fluid">
        <div class="crm-topbar">
            <div class="crm-title">
                <h2>Reply to {{ $enquiry->full_name }}</h2>
                <div class="crm-subtitle">{{ $enquiry->email }} · {{ $enquiry->company_name ?: $enquiry->business_name ?: 'GMS stone account request' }}</div>
            </div>
            <div class="crm-switcher">
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('gms-enquiries.index') }}">
                    <i class="fas fa-arrow-left"></i> Back
                </a>
                <a class="btn btn-outline-secondary btn-sm" href="{{ route('gms-enquiries.show', $enquiry->id) }}">
        ...
        <div class="crm-panel">
            <div class="crm-panel-head">
                ...
                <span class="crm-result-count">To: {{ $enquiry->email }}</span>
            </div>
            <form method="post" action="{{ route('gms-enquiries.reply.send', $enquiry->id) }}" class="crm-toolbar m-0 border-0 rounded-0">
                @csrf
